...
export const useResultContext = (result: ScanResult | null,filename: string | null): ResultContext => {
  return useMemo(() => {
    if (!result) {
      return {
        raw: null,
    ...
    const consolidatedContext = () => {
      switch(result.type){
        case 'file':
          return consolidateFileContext(result as FileObject, filename);
        case 'analysis':
          return consolidateAnalysisContext(result as AnalysisObject, filename);
        default:
          throw console.error("Unknown result type. Check again.");
      }
    }
...
function consolidateFileContext(result : FileObject, filename : string | null){
    const file = result;
    ...
    return {
      ...
      file: {
        ...
        //Cryptographic Hashes information
        hashes: {
          md5: a.md5 ?? undefined,
        ...
        //Overall threat severity statistics.
...
function consolidateAnalysisContext(result : AnalysisObject, filename : string | null){
    ...
    const threatLevel: 'safe' | 'warning' | 'danger' = malicious > 0 ? 'danger' : suspicious > 0 ? 'warning' : 'safe';
    ...
    return {
      ...
      analysis: {
        ...
        flaggedEngines: Object.entries(analysis.attributes.results)
          .filter(([_, engineResult]) => engineResult.category === 'malicious' || engineResult.category === 'suspicious')
          .map(([engineName, engineResult]) => ({
            engine: engineName,
            category: engineResult.category as 'malicious' | 'suspicious',
            method: engineResult.method,
            version : engineResult.engine_version,
            update : engineResult.engine_update,
            result: engineResult.result ?? undefined,
          })),
